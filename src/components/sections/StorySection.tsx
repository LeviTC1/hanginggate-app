    ...
    <SectionWrapper variant="light" py="lg" className={className}>
      <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_460px] lg:items-start">
        <RevealWrapper variant="stagger" className="space-y-5">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--gold-muted)]">Our story</p>
          ...
          {storyBlocks.map((copy) => (
            <article
              key={copy}
              className="rounded-2xl border border-[var(--border-default)] bg-[rgba(255,255,255,0.85)] p-6 shadow-[var(--shadow-sm)]"
            >
              <p className="text-[15px] leading-[1.8] text-[var(--text-secondary)]">{copy}</p>
            </article>
          ))}
          ...
          <div className="flex flex-wrap gap-3">
            {storyPills.map((pill) => (
              <span
                key={pill}
                className="rounded-full border border-[rgba(212,168,50,0.36)] bg-[rgba(212,168,50,0.12)] px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--gold-muted)]"
              >
                {pill}
              </span>
            ))}

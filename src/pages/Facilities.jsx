...
export default function Facilities() {
  return (
    <div>
      <SEO path="/facilities" description="Function rooms for up to 180 guests, free room hire. Weddings, christenings, funerals and birthdays. Fully accessible venue in Chapel-en-le-Frith, High Peak, Derbyshire." />
      ...
      <section style={{ maxWidth: '1000px', margin: '0 auto', padding: '64px 24px' }}>
        ...
        <div style={{ marginBottom: '56px' }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '28px', color: 'var(--surface-dark-2)', marginBottom: '16px' }}>
            Functions & Events
          </h2>
          ...
          <div style={{ marginTop: '24px', display: 'inline-block', backgroundColor: 'var(--surface-dark-2)', color: 'white', padding: '12px 24px', borderRadius: '4px', fontWeight: 600, fontSize: '15px' }}>
            All rooms are FREE to hire
          </div>
        </div>

        {/* Special Touches */}
        <div style={{ backgroundColor: 'var(--surface-warm)', borderRadius: '8px', padding: '36px', marginBottom: '48px' }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '26px', color: 'var(--surface-dark-2)', marginBottom: '20px' }}>
            Special Touches
          </h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '12px' }}>
            {touches.map(t => (
              <div key={t} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                ...
                <span style={{ color: 'var(--text-secondary)', fontSize: '15px', lineHeight: 1.6 }}>{t}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '12px', marginBottom: '24px' }}>
            {accessibility.map(a => (
              <div key={a} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', backgroundColor: 'var(--surface-card)', padding: '14px 16px', borderRadius: '6px', border: '1px solid var(--border-default)' }}>
                <span style={{ color: 'var(--gold)', marginTop: '2px', flexShrink: 0, fontSize: '18px' }}>✓</span>
                <span style={{ color: 'var(--text-secondary)', fontSize: '15px' }}>{a}</span>
              </div>
            ))}
          </div>

      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '64px 24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '16px', marginBottom: '56px' }}>
          {[
            ...
            <div key={f.label} style={{ backgroundColor: 'var(--surface-card)', border: '1px solid var(--border-default)', borderRadius: '8px', padding: '20px', textAlign: 'center' }}>
              <div style={{ fontSize: '28px', marginBottom: '8px' }}>{f.icon}</div>
              <div style={{ fontWeight: 700, color: 'var(--gold-light)', fontSize: '14px', letterSpacing: '0.03em' }}>{f.label}</div>
              <div style={{ color: 'var(--text-inverse)', fontSize: '12px', marginTop: '4px' }}>{f.sub}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '24px', marginBottom: '48px' }}>
          {menus.map((m, i) => (
            <div key={m.name} style={{
              ...
              border: `2px solid ${i === 1 ? 'var(--surface-dark-2)' : 'var(--border-default)'}`,
              borderRadius: '10px',
              overflow: 'hidden',
              position: 'relative',
            }}>
              {i === 1 && (
                <div style={{ backgroundColor: 'var(--surface-dark-2)', color: 'white', textAlign: 'center', fontSize: '12px', fontWeight: 600, padding: '6px', letterSpacing: '1px' }}>
                  POPULAR CHOICE
                </div>
              )}
              <div style={{ padding: '24px' }}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: '22px', color: 'var(--text-inverse)', marginBottom: '4px' }}>{m.name}</div>
                <div style={{ color: 'var(--gold)', fontWeight: 700, fontSize: '20px', marginBottom: '20px' }}>{m.price}</div>
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {m.items.map(item => (
                    <li key={item} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                      <span style={{ color: 'var(--gold)', flexShrink: 0, marginTop: '3px', fontSize: '12px' }}>✦</span>
                      <span style={{ color: 'var(--text-secondary)', fontSize: '14px', lineHeight: 1.5 }}>{item}</span>
                    </li>
                  ))}
                </ul>
                {m.note && <p style={{ color: 'var(--text-secondary)', fontSize: '12px', marginTop: '16px', fontStyle: 'italic' }}>{m.note}</p>}
              </div>
            </div>
          ))}
        </div>

        <div style={{ backgroundColor: 'var(--surface-dark-2)', borderRadius: '8px', padding: '32px', textAlign: 'center', color: 'white' }}>
          <p style={{ fontFamily: 'var(--font-display)', fontSize: '22px', color: 'var(--gold-light)', marginBottom: '8px' }}>
          ...
          <a href="tel:01298812776" style={{ backgroundColor: 'var(--gold-light)', color: '#1a1208', padding: '14px 32px', borderRadius: '4px', textDecoration: 'none', fontWeight: 700, fontSize: '16px', display: 'inline-block' }}>
